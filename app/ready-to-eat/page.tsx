import type { Metadata } from 'next';
import Link from 'next/link';
import { Breadcrumb } from '@/components/Breadcrumb';
import { ProductCard } from '@/components/ProductCard';
import { Section } from '@/components/Section';
import { readyToEat } from '@/data/products';

export const metadata: Metadata = {
  title: 'Ready To Eat',
  description:
    'Fully prepared South Indian dishes — poondu and sundaikai kulambu, dosa masal, mix veg curry, kambu and ragi kool. Heat, serve and enjoy.',
};

export default function ReadyToEatPage() {
  return (
    <>
      <Breadcrumb current="Ready To Eat" />

      <section className="pb-12 pt-8 text-center">
        <div className="shell">
          <h1 className="text-[34px] font-extrabold tracking-tight sm:text-[42px]">
            Ready To Eat
          </h1>
          <p className="mx-auto mt-4 max-w-[56ch] text-[15px] leading-relaxed text-ink-muted">
            Fully prepared dishes crafted with authentic recipes. Heat, serve and enjoy
            anytime — ideal for busy days when you refuse to compromise on health and
            taste.
          </p>
        </div>
      </section>

      <section className="pb-16">
        <div className="shell grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {readyToEat.map((product) => (
            <ProductCard key={product.slug} product={product} />
          ))}
        </div>
      </section>

      <Section tone="cream">
        <h2 className="section-title">Like to Cook It Yourself?</h2>
        <p className="section-sub">
          Our Ready To Cook range gives you the prep, and leaves the cooking to you.
        </p>
        <div className="mt-8 text-center">
          <Link href="/ready-to-cook" className="btn-gradient">
            View Ready To Cook
          </Link>
        </div>
      </Section>
    </>
  );
}
